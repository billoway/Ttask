...
//size是在这个epoll fd上能关注的最大socket fd数
static int
sp_create()
{
...
//epoll_ctl int epoll_ctl(int efd, intop, int fd, struct epoll_event*event);
static int 
...
//EPOLLIN:表示对应的文件描述符上有可读数据 EPOLLOUT:表示对应的文件描述符上可以写数据
static void 
...
sp_wait(int efd, struct event *e, int max)
{
    ...
    //等侍注册在efd上的socket fd的事件的发生，如果发生则将发生的sokct fd和事件类型放入到events数组中。
    //并且将注册在efd上的socket fd的事件类型给清空，所以如果下一个循环你还要关注这个socket fd的话，
    //则需要用epoll_ctl(efd,EPOLL_CTL_MOD,listenfd,&ev)来重新设置socket fd的事件类型
	int i;
	for (i=0;i<n;i++) {
		e[i].s = ev[i].data.ptr;

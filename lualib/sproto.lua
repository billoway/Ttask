...
-- creates a sproto object by a shcema text string(by calling parser.parse)
...
-- detect whether a type exist in sproto object.
...
-- ecodes a lua table with typename into a binary string.
...
-- The same with sproto:encode,but pack (compress) the results.
function sproto:pencode(typename, tbl)
	local st = querytype(self, typename)
	return core.pack(core.encode(st, tbl))
end
--The same with sproto.decode, but unpack the blob (generated by sproto:pencode) first.
...
-- RPC API ,not use host object
-- encode a request message with protoname.
...
-- RPC API ,not use host object
-- encode a response message with protoname.
...
-- RPC API ,not use host object
-- decode a request message with protoname.
...
-- RPC API ,not use host object
-- decode a response message with protoname.
...
--Create a table with default values of typename. They can  be nil , "REQUEST", or "RESPONSE".
function sproto:default(typename, type)
    print("sproto:default==>",typename,type)
	if type == nil then
...
-- RPC API
-- unpack and decode (sproto:pdecode) the binary string with type the host created (packagename).
-- If .type is exist ,it's a REQUEST message with .type, return "REQUEST", protoname,
-- message,responer, .ud.
-- The responser is a function for encode the response message.
-- The responer will be nil when .session is not exist.
--
-- If .type is not exist,it's a RESPONSE message for .session. Returns "RESPONSE",
-- .session, message, .ud.
...
-- RPC API
-- Creates a  function(protoname, message,session,ud) to pack and encode request message with sprotoobj.

...
local function resolve(content)
	if #content < DNS_HEADER_LEN then
		-- drop
		mtask.error("Recv an invalid package when dns query")
		return
	end
	...
	if answers_ipv4 then
		CACHE[QTYPE.A][question.name] = { answers = answers_ipv4, ttl = mtask.now() + ttl * 100 }
	end

	if answers_ipv6 then
		CACHE[QTYPE.AAAA][question.name] = { answers = answers_ipv6, ttl = mtask.now() + ttl * 100 }
	end
	...
	if question.name ~= resp.name then
		mtask.error("Recv an invalid name when dns query")
	end
...
local function lookup_cache(name, qtype, ignorettl)
	...
	if result then
		if ignorettl or (result.ttl > mtask.now()) then
...
local function suspend(tid, name, qtype)
	...
	mtask.fork(function()
		...
		if req then
			-- cancel tid
			mtask.error(string.format("DNS query %s timeout", name))

...
function provider.query(name)
	local s = svr[name]
	if s.queue then
		table.insert(s.queue, mtask.response())
	else
		if s.address then
			return mtask.ret(mtask.pack(s.address))
		else
			error(s.error)
		end
	end
...
function provider.launch(name, code, ...)
	...
	if s.booting then
		...
		s.booting = true
		local err
		local ok, addr = pcall(mtask.newservice,"service_cell", name)
		if ok then
		...
		if ok then
			...
			s.queue = nil
			mtask.ret(mtask.pack(addr))
		else
			if addr then
				mtask.send(addr, "debug", "EXIT")
			end
			s.error = err
			for _, resp in ipairs(s.queue) do
				resp(false)
...
function provider.test(name)
	local s = svr[name]
	if s.booting then
		mtask.ret(mtask.pack(nil, true))	-- booting
	elseif s.address then
		mtask.ret(mtask.pack(s.address))
	elseif s.error then
		error(s.error)
	else
		mtask.ret()	-- nil

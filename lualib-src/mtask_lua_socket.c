...
lunpack(lua_State *L) {
    ...
    if (message->type == MTASK_SOCKET_TYPE_UDP) {
        int addrsz = 0;
        const char * addrstring = mtask_socket_udp_address(message, &addrsz);
        if (addrstring) {
...
lsendlow(lua_State *L) {
    ...
    int sz = 0;
    void *buffer = get_buffer(L, 2, &sz);
    int err = mtask_socket_send_lowpriority(ctx, id, buffer, sz);
    lua_pushboolean(L, !err);
    return 1;
}
...
LUAMOD_API int
luaopen_mtask_socketdriver(lua_State *L) {
    luaL_checkversion(L);
    luaL_Reg l[] = {
        { "buffer", lnewbuffer },
        { "push", lpushbuffer },

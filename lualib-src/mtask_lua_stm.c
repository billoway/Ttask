...
// msg should alloc by mtask_malloc
static struct stm_copy *
stm_newcopy(void * msg, int32_t sz) {
    struct stm_copy * copy = mtask_malloc(sizeof(*copy));
    copy->reference = 1;
    copy->sz = sz;
    copy->msg = msg;
...
stm_new(void * msg, int32_t sz) {
    struct stm_object * obj = mtask_malloc(sizeof(*obj));
...
lnewwriter(lua_State *L)
{
    ...
    box->obj = stm_new(msg,(int32_t)sz);
...
lupdate(lua_State *L)
{
    ...
    stm_update(box->obj, msg, (int32_t)sz);
